...
<div x-data="basic">
    ...
    <div class="panel" id="display_data">
        <h5 class="font-semibold text-lg dark:text-white-light">Log Orders</h5>
...
<script>
                        ...
                        if (data && data.status === "200" && data.data && data.data.length > 0) {
                            var tableRows = data.data.map(function(item) {
                                return "<tr>" +
                                    ...
                                    "<td><button class='btn btn-primary' onclick='viewOrderDetails(" + item.order_id + "," + "\"" + item.source + "\"" + ")'>View Details</button></td>" +
                                    "</tr>";
                            });
                            ...
                            self.datatable = new simpleDatatables.DataTable('#trans_tb', {
                                data: {
                                    headings: ["Product Name", "Ordered At", "Amount", "Quantity", "Action"],
                                },
                                sortable: true,
                                searchable: true,
                                perPage: 20,
   ...
   <center><img src="https://cdn-icons-png.flaticon.com/512/5741/5741333.png"  height="150" width="150"></center>
    ...
    function viewOrderDetails(order_id, source) {
        if(source === "post"){
            window.location.href = "log-post-order-details?order_id=" + order_id;
        }else{
            window.location.href = "log-order-details?order_id=" + order_id;
        }
    }
</script>
...
<a href="https://wa.me/+2349117575162" class="float-message float2" target="_blank"> 
