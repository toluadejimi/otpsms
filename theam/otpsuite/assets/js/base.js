...
function AddtoHome(time, once) {
    if (once) {
        var AddHomeStatus = localStorage.getItem("OtpsuiteAddtoHome");
        if (AddHomeStatus === "1" || AddHomeStatus === 1) {
            // already showed up
        }
        else {
            localStorage.setItem("OtpsuiteAddtoHome", 1)
...
//-----------------------------------------------------------------------
// Dark Mode
var checkDarkModeStatus = localStorage.getItem("OtpsuiteDarkmode");
var switchDarkMode = document.querySelectorAll(".dark-mode-switch");
var pageBodyActive = pageBody.classList.contains("dark-mode");

// Check if enable as default
if (Otpsuite.Dark_Mode.default) {
    pageBody.classList.add("dark-mode");
}

// Local Dark Mode
if (Otpsuite.Dark_Mode.local_mode.enable) {
    var nightStart = Otpsuite.Dark_Mode.local_mode.start_time;
    var nightEnd = Otpsuite.Dark_Mode.local_mode.end_time;
...
if (Otpsuite.Dark_Mode.auto_detect.enable)
...
switchDarkMode.forEach(function (el) {
    el.addEventListener("click", function () {
        var darkmodeCheck = localStorage.getItem("OtpsuiteDarkmode");
        var bodyCheck = pageBody.classList.contains('dark-mode');
        if (darkmodeCheck === 1 || darkmodeCheck === "1" || bodyCheck) {
            pageBody.classList.remove("dark-mode");
            localStorage.setItem("OtpsuiteDarkmode", "0");
            switchDarkModeCheck(false);
        }
        else {
            pageBody.classList.add("dark-mode")
            switchDarkModeCheck(true);
            localStorage.setItem("OtpsuiteDarkmode", "1");
        }
    })
})

// Button-based trigger (no checkbox)
...
function syncThemeIcon() {
   ...
   if (!themeBtn) return;
    ...
    if (pageBody.classList.contains("dark-mode")) {
        themeBtn.innerHTML = '<ion-icon class="icon" name="sunny-outline"></ion-icon>';
    } else {
        themeBtn.innerHTML = '<ion-icon class="icon" name="moon-outline"></ion-icon>';
    }
}
...
if (document.querySelector(".offcanvas") === null) {
    ...
    var CookiesStatus = localStorage.getItem("OtpsuiteCookiesStatus")
    ...
    document.querySelectorAll(".accept-cookies").forEach(function (el) {
        el.addEventListener("click", function () {
            localStorage.setItem("OtpsuiteCookiesStatus", "1")
        })
    })
}
...
function testMode() {
    ...
    console.clear();
    console.log("%cOtpsuite", "font-size: 1.3em; font-weight: bold; color: #FFF; background-color: #6236FF; padding: 10px 120px; margin-bottom: 16px;")
    ...
    testModeMsg(Otpsuite.PWA.enable, "PWA")
    testModeMsg(Otpsuite.Dark_Mode.default, "Set dark mode as default theme")
    testModeMsg(Otpsuite.Dark_Mode.local_mode.enable, "Local dark mode (between " + Otpsuite.Dark_Mode.local_mode.start_time + ":00 and " + Otpsuite.Dark_Mode.local_mode.end_time + ":00)")
    testModeMsg(Otpsuite.Dark_Mode.auto_detect.enable, "Auto detect dark mode")
    testModeMsg(Otpsuite.RTL.enable, "RTL")
    testModeMsg(Otpsuite.Test.enable, "Test mode")
    testModeMsg(Otpsuite.Test.alert, "Test mode alert")
    ...
    testModeMsg(Otpsuite.Animation.goBack, "Go Back")
}
function themeTesting() {
    var word = Otpsuite.Test.word;
    var value = "";
