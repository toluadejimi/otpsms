...
function copyOtpToClipboard(textareaId) {
  // Convert to string and ensure it's not null or undefined
  // Get textarea from dom
  var textareaWithSms = document.getElementById(textareaId);
  
  var text = String(textareaWithSms.innerHTML);
  if (!text) {
    return;
  }
...
function countdownTimer(milliseconds, elementId) {
  ...
  if (element) {
    ...
    $(element).empty().append('<i class="ri-time-line me-1"></i> 00:00');
  ...
  Notiflix.Confirm.show(
    ...
    function okCb() {
      ...
      $("#" + elements)
        .prop("disabled", true)
        .html(
          '<span class="animate-spin border-2 border-danger border-l-transparent rounded-full w-4 h-4 ltr:mr-1 rtl:ml-1 inline-block align-middle"></span>Cancel'
        );
      $.ajax({
        type: "GET",
...
function getImageUrlBasedOnDigits(inputString) {
  ...
  if (firstTwoDigits === "91") {
    ...
    return "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR_u8bkK6j3_AQyAuoynuI2f1opZBlc4vKjCg&usqp=CAU";
  }
}
...
function checkOrder() {
  ...
  $.ajax({
    type: "GET",
    url: "api/service/ActiveNumberUSAOnly",
    data: params,
          ...
          <div class="mt-3 mb-2 border-0">
              <div>
                  ...
                  <div class="d-flex align-items-center mb-3">
                      <img
                          id="flag_${item.id}"
                          src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ6os1cbe84_XbWy76cW9R8uszpM9ASmrG17Q&usqp=CAU"
                          width="45"
                          height="45"
                          class="me-2"
                          alt="flag"
                      ...
                      <div class="flex-grow-1">
                          <div class="d-flex align-items-center">
                              <p id="num" class="mb-0">+${item.number}</p>
                              <span
                                  id="copy"
                          ...
                          <p id="service-name" class="small mb-0">
                              ${item.app} - ₦${item.amount}
                          </p>
                      </div>
                  </div>
                  ...
                  <div class="position-relative mb-3">
                      ...
                      <span
                          id="t_${item.id}"
                          class="position-absolute bottom-0 end-0 mb-2 me-2 px-3 py-1 rounded-pill bg-light text-dark small"
                      >
                          <i class="ri-time-line me-1"></i>20:00
                      </span>
                  </div>
                  ...
                  <div class="text-center">
                      <div class="d-flex justify-content-center">
                          <button
                              onclick="copyOtpToClipboard('sms_${item.id}')"
                              class="btn btn-outline-primary rounded-pill me-2"
                          >
                              <i class="ri-file-copy-2-line me-2"></i> Copy OTP
                          </button>

                          <button
                              id="cancel_${item.id}"
                              onclick="model_cancle('${item.id}', 'cancel_${item.id}','${item.number}')"
